...
class DefaultCalculator implements Calculator
{
    public static function getAttribute(string $attribute, CartItem $cartItem)
    {
        ...
        switch ($attribute) {
            ...
            // Nuevos atributos para precios al detal
            case 'priceDetalTotal':
                return round($cartItem->priceDetal * $cartItem->qty, $decimals);

            case 'subtotalDetal':
                return max(round($cartItem->priceDetalTotal - $cartItem->discountTotal, $decimals), 0);

            case 'taxDetal':
                return round($cartItem->subtotalDetal * ($cartItem->taxRate / 100), $decimals);

            case 'totalDetal':
                return round($cartItem->subtotalDetal + $cartItem->taxDetal, $decimals);
